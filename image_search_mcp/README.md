...
本 MCP 服务使用 **NCT-CRC-HE-100K** 数据集作为病理图谱库的数据源。

#### 数据集概述

- **全称**: NCT-CRC-HE-100K (National Center for Tumor diseases - Colorectal Cancer - Hematoxylin-Eosin - 100K)
- **来源**: [Kaggle](https://www.kaggle.com/datasets/imrankhan77/nct-crc-he-100k)
- **类型**: 结直肠癌病理切片图像数据集
- **规模**: 约 **100,000 张**病理切片图像
- **格式**: TIF 格式，高分辨率病理切片图像
- **类别**: 9 个组织类型类别
...
| 类别代码 | 类别名称 | 描述 | 图片数量（约） |
|---------|---------|------|--------------|
| **ADI** | Adipose | 脂肪组织 | ~10,000 |
| **BACK** | Background | 背景/空白区域 | ~10,000 |
| **DEB** | Debris | 坏死碎片 | ~11,000 |
| **LYM** | Lymphocytes | 淋巴细胞 | ~11,000 |
| **MUC** | Mucus | 粘液 | ~8,000 |
| **MUS** | Muscle | 平滑肌 | ~13,000 |
| **NORM** | Normal | 正常粘膜组织 | ~8,000 |
| **STR** | Stroma | 肿瘤基质 | ~10,000 |
| **TUM** | Tumor | 腺癌上皮（肿瘤） | ~14,000 |

**总计**: 约 100,000 张图像

#### 数据集特点

1. **高质量标注**: 所有图像都经过病理专家标注，类别准确可靠
2. **多样性**: 涵盖正常组织、肿瘤组织、炎症反应等多种病理状态
3. **标准化**: 统一使用 H&E（苏木精-伊红）染色，便于模型学习
4. **高分辨率**: 图像分辨率高，保留了丰富的组织细节信息
...
# 使用 kagglehub（推荐）
import kagglehub
path = kagglehub.dataset_download("imrankhan77/nct-crc-he-100k")
print(f"数据集路径: {path}")
```
...
**推荐配置**: 每个类别 **20-200 张图片**，总计 **180-1800 张**

#### 为什么在开发测试阶段使用少量图片？

1. **快速迭代开发**
   - ⚡ **索引构建速度快**: 1800 张图片索引构建仅需 **30-60 分钟**（GPU）
   - ⚡ **服务器启动快**: 模型加载和数据库连接更快
   - ⚡ **测试反馈及时**: 可以快速验证功能是否正常

2. **资源消耗低**
   - 💾 **磁盘空间**: 约 270 MB（vs 完整数据集 15+ GB）
   - 💾 **数据库大小**: 约 7 MB（vs 完整数据集 500+ MB）
   - 💾 **内存占用**: 2-4 GB（vs 完整数据集可能需要 8+ GB）

3. **功能验证充分**
   - ✅ 1800 张图片足以验证搜索算法的正确性
   - ✅ 9 个类别覆盖了所有病理类型
   - ✅ 可以测试各种边界情况和错误处理
...
python build_atlas.py \
    --source_dir /path/to/NCT-CRC-HE-100K \
    --target_dir ./atlas_data_test \
    --images_per_category 20  # 总计 180 张

# 中型测试（推荐开发使用）
python build_atlas.py \
    --source_dir /path/to/NCT-CRC-HE-100K \
    --target_dir ./atlas_data_dev \
    --images_per_category 200  # 总计 1800 张
```

### 🚀 生产环境：使用完整数据集

**强烈建议**: 在生产环境中使用 **完整数据集的所有图片**（约 100,000 张）

#### 为什么生产环境要使用完整数据集？

1. **搜索准确度最大化**
   - 🎯 **更多参考病例**: 10 万张图片提供更丰富的参考库
   - 🎯 **覆盖更多变异**: 涵盖更多组织形态变异和病理状态
   - 🎯 **提高匹配精度**: 更大的样本空间提高相似病例匹配的准确性

2. **临床实用性**
   - 🏥 **真实场景**: 生产环境需要处理各种复杂病理情况
   - 🏥 **罕见病例**: 完整数据集包含更多罕见或边缘病例
   - 🏥 **诊断参考**: 为临床诊断提供更全面的参考信息

3. **系统稳定性**
   - 🔒 **性能优化**: 大规模数据可以更好地测试和优化系统性能
   - 🔒 **负载测试**: 验证系统在高负载下的稳定性
   - 🔒 **生产就绪**: 确保系统能够处理生产环境的实际需求

#### 生产环境配置

```bash
# 使用完整数据集构建索引
python indexer.py --atlas_dir /path/to/NCT-CRC-HE-100K/versions/1/NCT-CRC-HE-100K
...
| 指标 | 开发测试（1800张） | 生产环境（10万张） |
|------|------------------|------------------|
| **索引构建时间** | 30-60 分钟 | 4-8 小时（GPU） |
| **数据库大小** | ~7 MB | ~500 MB - 1 GB |
| **搜索速度** | < 100ms | < 200ms |
| **内存占用** | 2-4 GB | 4-8 GB |
| **搜索准确度** | 良好 | 优秀 |
...
开发阶段                   测试阶段                   生产阶段
   ↓                         ↓                         ↓
20-200张/类             500-1000张/类              全部图片
(180-1800张)            (4500-9000张)              (~10万张)
   ↓                         ↓                         ↓
功能验证                性能测试                  生产部署
快速迭代                压力测试                  稳定运行
```
...
**开发测试阶段**（推荐每个类别 200 张，总计 1800 张）：

```bash
# 1. 构建图谱（每个类别200张图片，用于开发测试）
python build_atlas.py \
...
**生产环境**（使用完整数据集，约 10 万张图片）：

```bash
# 直接使用完整数据集构建索引（无需 build_atlas.py）
python indexer.py --atlas_dir /path/to/NCT-CRC-HE-100K/versions/1/NCT-CRC-HE-100K

# 注意：完整数据集索引构建需要数小时，建议：
# - 使用 GPU 加速
# - 在非高峰期构建
# - 确保有足够的磁盘空间（15+ GB）
```
...
**注意**: 这是开发测试阶段的配置。生产环境建议使用完整数据集（约 10 万张图片），详见[数据集介绍](#数据集介绍)章节。
